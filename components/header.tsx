...
export default function Header() {
  ...
  return (
    <header className="z-[999] relative">
      <motion.div
        className="fixed top-0 left-1/2 h-[4.5rem] w-full rounded-none border border-white/20 border-opacity-40 bg-white/80 dark:bg-[#1d1d20] bg-opacity-90 shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[46rem] sm:rounded-full"
        initial={{ y: -100, x: "-50%", opacity: 0 }}
        animate={{ y: 0, x: "-50%", opacity: 1 }}
      ></motion.div>

      <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0 w-full sm:w-auto px-4 sm:px-0">
        {/* Logo Section */}
        <motion.div
        ...
        {/* Desktop Navigation */}
        <ul className="hidden sm:flex items-center justify-center gap-1 text-[0.9rem] font-medium text-gray-600 dark:text-gray-400 ml-8">
          {links.map((link) => (
            <motion.li
            ...
            >
              <Link
                className={clsx(
                  "flex w-full items-center justify-center px-3 py-3 rounded-full transition-all duration-200 hover:text-gray-900 dark:hover:text-white/90",
                  {
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            className="fixed top-[4.5rem] left-0 right-0 bg-white/95 dark:bg-[#1d1d20]/95 backdrop-blur-md border-t border-gray-200 dark:border-gray-700 sm:hidden z-50"
